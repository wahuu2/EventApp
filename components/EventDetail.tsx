...
const EventDetail = async ({ slug }: { slug: string }) => {
  ...
  if (!eventData?.description) return notFound();

  const { description, image, overview, date, time, location, mode, agenda, audience, organizer, tags } = eventData;
  const bookings = 10;
        ...
        <div className="events">
          {similarEvents.length > 0 &&
            similarEvents.map((similarEvent: IEvent) => {
              const plainEvent = { ...similarEvent, _id: similarEvent._id.toString() };
